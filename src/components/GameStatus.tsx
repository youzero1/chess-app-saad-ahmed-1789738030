...
export function GameStatus({ status, turn, onNewGame }: GameStatusProps) {
  ...
  if (status === "checkmate") {
    banner =
      turn === "w"
        ? { text: "Checkmate — AI wins", className: "bg-accent-dark text-blue-50" }
        : { text: "Checkmate — You win! 🏆", className: "bg-board-dark text-blue-50" };
    turnText = "Game over";
  } else if (status === "stalemate") {
    banner = { text: "Stalemate — Draw", className: "bg-frame text-blue-50" };
    turnText = "Game over";
  } else if (status === "draw") {
    banner = { text: "Draw", className: "bg-frame text-blue-50" };
    turnText = "Game over";
  } else {
